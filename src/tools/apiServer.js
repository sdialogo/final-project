...
server.post("/addDevPlan/", function(req, res, next) {
  const error = validateDevPlan(req.body);
  if (error) {
...
function validateDevPlan(devPlan) {
  if (!devPlan.title) return "Title is required.";
  if (!devPlan.employeeId) return "Employee ID is required.";
  if (!devPlan.description) return "Description is required.";
  return "";
}
